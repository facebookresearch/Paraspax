...
%Load logfile
subj.logtxt = fopen(fullfile('subjects',subj.logtxtName),'a');
if subj.logtxt == -1
   error('Cannot open log file!'); 
end

%Iterate listener position
if subj.expCompletePerPosition(subj.currentListenerPosition)
    subj.currentListenerPosition = subj.currentListenerPosition+1;
end

%Variable for start
firstTrialForThisPosition = true;
...
function midiTimerFcn(~,~)
    ...
    if size(msg,1) > 0 %Not empty
        ...
        if (sum(midiChannelID == 1)/size(msg,1)) == 1
            ...
            elseif (subj.trial == (subj.nTrialsPerPosition * sum(subj.expCompletePerPosition))) && firstTrialForThisPosition
                firstTrialForThisPosition = false;
                disp('Test continued');
                runTrial();
                ...
                if subj.trial < (subj.nTrialsPerPosition * subj.currentListenerPosition)
                    runTrial();
...
function endExperiment()
    ...
    %Set current position to completed
    subj.expCompletePerPosition(subj.currentListenerPosition) = 1;
    ...
    if subj.trial == subj.nTrials %If entire test is completed
        %Set to completed
        subj.expComp = 1;
    ...
    else %If just position completed
        
        disp('Position complete...');
        %Play message indicating that experiment is complete
        changePosition = audioread('ChangePosition.wav');
        %AKio(expComplete,2,[],sh.io.fs,sh.io.playDev)

        outputChannel = 2;
        %Audioplayer always plays both channels. Add zeros...
        if outputChannel == 1 || outputChannel == 3
            changePosition = [changePosition,zeros(length(changePosition),1)];
        end
        if outputChannel == 2 || outputChannel == 4
            changePosition = [zeros(length(changePosition),1),changePosition];
        end

        %Create audioPlayerObject and play
        audioPlayerObj = audioplayer(changePosition,sh.io.fs,sh.io.nBits,sh.io.outputIdMatrix(outputChannel));
        play(audioPlayerObj)
