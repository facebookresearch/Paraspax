...
%
% function opti2PyBinSim_Log_Positions(optiTimer);
%
...
function opti2PyBinSim_Log_Position(optiTimer)
...
function optiTimerFcn(~,~)
    ...
    %---- LOG FILE ----
    %If test running, write log file
    if subj.trial > 0 && ~subj.expComp && ~subj.expCompletePerPosition(subj.currentListenerPosition)
       
        currentTime = clock;
        logMessage = ['Trial: ',num2str(subj.trial),', ',...
                    'RV: ',num2str(subj.rvsequ(subj.trial)),', ',...
                    'Source: ',num2str(subj.sourceSequ(subj.trial)),', ',...
                    'RelAZ: ',num2str(relativeAngles(1)),', ',...
                    'RelEL: ',num2str(relativeAngles(2)),', ',...
                    'X: ',num2str(lpPar(1)),', ',...
                    'Y: ',num2str(lpPar(2)),', ',...
                    'Z: ',num2str(lpPar(3)),', ',...
                    'Time: ',num2str(currentTime(4)),':',num2str(currentTime(5)),':',num2str(currentTime(6))];
                
        fprintf(subj.logtxt, '%s\n', logMessage);
