...
%
% script renderRoomABRIR_pyBinSim_splitBRIR_Az4EL10_XYZ025
%
...
if recPosID < 10
    fileName = ['RoomA_SDM_MeasLoc_0',num2str(recPosID),'_SourceNum_',num2str(srcPosID),'_az00_el00'];
else
    fileName = ['RoomA_SDM_MeasLoc_',num2str(recPosID),'_SourceNum_',num2str(srcPosID),'_az00_el00'];
end
load(fileName)
...
%HRTFs in SH domain
psx.brir.HRTFs = importdata('KEMAR_HRIRs_Meas_ALFE_sfd_N35.mat');

%Load corresponding diffuse field response / common transfer function
psx.brir.DFR = importdata('DFR_KEMAR_Meas_ALFE.mat');
...
psx.brir.binNoise = importdata('BinauralWhiteNoise_KEMAR_Meas_ALFE_Artificial_NoIC.mat');
...
if recPosID < 10
    psxName = ['psx_RoomA_RecPos_0',num2str(recPosID),'_SrcPos_',num2str(srcPosID)];
else
    psxName = ['psx_RoomA_RecPos_',num2str(recPosID),'_SrcPos_',num2str(srcPosID)];
end
