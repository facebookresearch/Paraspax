...
% study, see [1]. Description see PlausibilityExperiment_Docs. 'Positions' 
% is for a 3-DoF plausibility study.
%
...
winopen('D:\Paraspax\PyBinSimProjects\startPyBinSim_RoomA_SDM10_Az4EL10_XYZ100.bat');
...
subj.ID = 'ADRA_PSX_ISM_POSITION_ID'; 
%!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
subj.run = 0;   
...
sh.nTestSignals = 30;
sh.nSources = 4;
sh.nPositions = 4; %Number of listener positions
sh.pybinPath = 'D:\Paraspax\PyBinSimProjects\RoomA_SDM10_Az4El10_XYZ100';
sh.expPath = pwd;
...
if subj.run == 0
    genSubjCond_Positions(); %Works with global variable subj
end
...
opti2PyBinSim_Log_Position(optiTimer)
disp('Experiment ready to start...');

%% (5) - Start Experiment Position 1
...
runExperiment_Positions(midiTimer);

%% (6) - Start Experiment Position 2

runExperiment_Positions(midiTimer);

%% (7) - Start Experiment Position 3

runExperiment_Positions(midiTimer);

%% (8) - Start Experiment Position 4
